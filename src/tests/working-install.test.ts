...
describe.each`
    title                  | origin            | originString
    ${'with absolute URL'} | ${{ port: 2001 }} | ${'http://localhost:2001'}
    ${'with relative URL'} | ${undefined}      | ${''}
`('working install $title', ({ origin, originString }) => {
    beforeAll((done) => {
    ...
    test.each([{ mode: 'regular' }, { mode: 'deprecated' }])(
        'install root ($mode)',
        async ({ mode }: { mode: 'regular' | 'deprecated' }) => {
            const events = []
            await doInstall(
                {
                    modules: ['root'],
...
//# sourceURL=${originString}/api/assets-gateway/raw/package/YQ==/1.0.0/`)
    ...
    test('install a', async () => {
        ...
        expect(
            events.filter((e) => e instanceof InstallDoneEvent),
        ).toHaveLength(1)
        setTimeout(() => {
