...
const origin = 'http://localhost:2001'
beforeAll((done) => {
    installPackages$([
        './.packages/root.zip',
        './.packages/a.zip',
...
test('fetch script', async () => {
    ...
    expect(resp).toEqual({
        ...
        version: '1.0.0',
        assetId: 'YQ==',
        url: `${origin}/api/assets-gateway/raw/package/YQ==/1.0.0/folder/add-on.js`,
        content: `window.a.addOn = ['add-on']

//# sourceURL=${origin}/api/assets-gateway/raw/package/YQ==/1.0.0/folder/`,
    })
})
...
//# sourceURL=${origin}/api/assets-gateway/raw/package/YQ==/1.0.0/`)
})
...
test('install style sheet', async () => {
    ...
    expect(link.id).toBe(
        `${origin}/api/assets-gateway/raw/package/YQ==/1.0.0/style.css`,
    )
